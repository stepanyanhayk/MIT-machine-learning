...
def compute_probabilities(X, theta, temp_parameter):
    ...
    """
    c = (theta @ X.T / temp_parameter).max(axis=0)
    denom = np.exp(theta @ X.T / temp_parameter - c).sum(axis=0)
    H = (1 / denom) * np.exp(theta @ X.T / temp_parameter - c)
    return H
...
def compute_cost_function(X, Y, theta, lambda_factor, temp_parameter):
    ...
    probabilities = np.choose(Y, compute_probabilities(X, theta, temp_parameter))
    log_expression = np.sum(np.log(probabilities))
    return (-1 / X.shape[0]) * log_expression + regularization
...
def run_gradient_descent_iteration(X, Y, theta, alpha, lambda_factor, temp_parameter):
    ...
    gradient = (-1 / (temp_parameter * n)) * ((M - probabilities) @ X) + lambda_factor * theta
...
def compute_test_error_mod3(X, Y, theta, temp_parameter):
    ...
    Y_pred_mod3 = np.remainder(Y_pred, 3)
...
def compute_probabilities_kernel(alpha_matrix, kernel_matrix, temp_parameter):
    factor = alpha_matrix @ kernel_matrix / temp_parameter
    c = np.max(factor, axis=0)
    H = np.exp(factor - c) / np.exm(factor - c).sum()
    return H
